...
class Model(Module):
    ...
    def forward(self, x):
        x = self.linear1(x)
        # if all elements in x = nan, then stop execution
        if np.isnan(x.data).all():
            print('x is nan')
            exit()
        x = x.relu()
        x = self.linear2(x)
...
for epoch in range(1):
    ...
    for batch in dataloader:
        x, y = zip(*batch)
        x = Tensor(np.array([img.numpy().flatten() for img in x]) / 255, requires_grad=True) # Normalize input
        y = Tensor(np.array(y), requires_grad=False)  # Use integer labels for CrossEntropyLoss
...
for batch in dataloader:
    x, y = zip(*batch)
    x = Tensor(np.array([img.numpy().flatten() for img in x]) / 255, requires_grad=False)  # Normalize input
    y = np.array(y)
